...
<section id="galeri-detail" class="py-5">
  <div class="container" data-aos="fade-up">
    <h3 class="mb-4">{{ $galeri->title }}</h3>
    ...
    <div class="card shadow mb-4">
      <div class="card-body">
        ...
        <ul class="list-unstyled mb-3">
          <li><strong>Kategori:</strong> {{ $galeri->category->name ?? '-' }}</li>
          <li><strong>Album:</strong> {{ $galeri->album->name ?? '-' }}</li>
          <li><strong>Uploader:</strong> {{ $galeri->uploader ?? '-' }}</li>
          <li><strong>Status:</strong> 
            <span class="badge bg-{{ $galeri->status === 'published' ? 'success' : ($galeri->status === 'draft' ? 'warning' : 'secondary') }}">
              {{ ucfirst($galeri->status) }}
            </span>
          </li>
          <li><strong>Tanggal Diambil:</strong> {{ $galeri->taken_at ? \Carbon\Carbon::parse($galeri->taken_at)->format('d M Y') : '-' }}</li>
          <li><strong>Featured:</strong> {{ $galeri->is_featured ? 'Ya' : 'Tidak' }}</li>
        </ul>

        <a href="{{ route('galeri.index') }}" class="btn btn-secondary mt-2">← Kembali</a>
      </div>
    </div>
  </div>
</section>
<!-- End Galeri Detail Section -->
