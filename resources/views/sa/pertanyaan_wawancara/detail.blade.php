...
<div class="card">
    <div class="card-body">
        <h5 class="card-title">Detail Pertanyaan Wawancara</h5>

        @if(session('success'))
        <div class="alert alert-success">{{ session('success') }}</div>
        @endif

        <form action="{{ route('sa.pertanyaan_wawancara.store') }}" method="POST" class="mb-4">
            @csrf
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @foreach($pertanyaans as $no => $item)
                <tr>
                    <td>{{ $no+1 }}</td>
                    <td>{{ $item->unitKompetensi->judul_unit ?? '-' }}</td>
                    <td>{{ $item->pertanyaan }}</td>
                    <td>
                        <form action="{{ route('sa.pertanyaan_wawancara.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus pertanyaan ini?')">
                            @csrf
                            @method('DELETE')
